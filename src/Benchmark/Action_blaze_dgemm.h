...
struct Action_blaze_dgemm : public ActionBase
{
	BRAINTWISTER_RECORD( Settings, \
	    (( int, number_of_threads, 1 )) \
	);

	Action_blaze_dgemm(ptree const& tree)
     : settings(tree)
    {}

	std::string name() const { return "blaze_dgemm"; }
        ...
        BrainTwister::Benchmark const& benchmark) const
    {
		...
		auto result = benchmark.benchIt([&](){
			C = (refA * refB).template execute<Blaze>();
		});
